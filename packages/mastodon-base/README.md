...
Shared support for Mastodon-compatible ActivityPlug adapters.
...
Streaming requires an injected `webSocket` factory. The adapter does not use a
global WebSocket implementation because server applications must apply their
own egress policy and DNS pinning before connecting to token-bearing URLs.
