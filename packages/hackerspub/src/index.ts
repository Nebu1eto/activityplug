...
export function createHackersPubAdapter(
  ...
  return {
    ...
    instances: {
      detect: async (_input, context) => getInstanceProfile(context),
      getProfile: async (_input, context) => getInstanceProfile(context),
    },
    accounts: {
...
async function getInstanceProfile(context: AdapterOperationContext): Promise<InstanceProfile> {
