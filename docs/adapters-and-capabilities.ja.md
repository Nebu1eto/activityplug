...
[English](adapters-and-capabilities.md) |
[한국어](adapters-and-capabilities.ko.md) | 日本語

ActivityPlug のアダプタは特定のサーバファミリの API を共通のクライアント
契約へ変換します。この契約はすべてのサーバがすべての操作を実装することを
意味しません。capability の判定は、選択したアダプタとインスタンスで
...
パッケージです。呼び出し元がアダプタ identity、対応ソフトウェア名、
ファミリ固有の動作を指定するため、汎用的なソフトウェア自動検出には使い
ません。
...
MIME type の上限を宣言できます。サーバが表現できない入力を組み立てる前に
制約を確認してください。
...
以下の表はアダプタのおおまかな動作を示します。「対応」はその機能グループ
をマッピングするという意味であり、グループ内のすべての操作へ無条件に対応
するという意味ではありません。操作別・バージョン別の判定には有効な
...
これらの失敗は `null`、空のコレクション、暗黙に変更した入力ではなく、
型付きエラーとして返されます。
...
アプリケーションが対応するソフトウェアファミリの具体的なパッケージを使い
ます。複数インスタンスを扱うサービスでは必要なアダプタをすべて登録し、
信頼できる検出結果から選択します。アダプタ自身のインスタンス検出を実行
せず、信頼できないソフトウェア名の文字列からアダプタを選択しないで
ください。
...
[ストリーミングとメディア](streaming-and-media.ja.md)を、エラーモデルは
[エラーとトラブルシューティング](errors-and-troubleshooting.ja.md)を
参照してください。
