...
`@activityplug/core`
の正規化された契約へマッピングします。対象ソフトウェアが提供し、必要な意味を失わずにマッピングできる動作
だけを実装してください。
...
補完します。リモート API がポータブルな動作を提供しないと判明している場合は、
理由を付けて明示的に `unsupported` と判定してください。
...
この例は参照の構造を示しています。本番用マッピングではリモート
レスポンス全体を検証し、必須の正規化フィールドをすべて設定する必要が
あります。エンティティを作成する前にスキーマまたは明示的なバリデータを
使ってください。必須のリモートフィールドが欠けている場合は、空のポータブルな値で
埋めるのではなく `REMOTE_PROTOCOL_ERROR` または `REMOTE_ERROR` として
扱います。

診断に有用な場合はリモートペイロードを `raw` に保持します。安定した
...
と `before` に対応します。ポータブルな上限 100 と、それより低いリモート
上限を適用してください。
...
通常のサービス入力で呼び出し元がアクセストークンを直接渡すことを想定
しないでください。使用前にセッションが選択したアダプターと origin に属する
ことを確認してください。
...
バージョン範囲には制約情報を使います。文書化されていない一部の入力だけを受け付けるにもかかわらず、操作全体
を対応済みと宣言しないでください。
...
最も具体的なポータブルコードを持つ `ActivityPlugError` を送出します。

 -  無効な呼び出し元入力には `VALIDATION_FAILED`
 -  資格情報の状態には `AUTH_REQUIRED` または `AUTH_EXPIRED`
 -  利用できないポータブルな動作には `UNSUPPORTED_OPERATION`
...
判明している場合はアダプター、origin、操作、capability の情報を
コンテキストに含めます。有用であれば元のエラーを `cause` として保持
しますが、`message`、`context.raw`、ログにトークンやリモートシークレット
を含めないでください。


検証済みファクトリーがある場合にのみストリーミングを追加
--------------------------------------------------------
...
ください。代表的なマッピングケース 1 件と不正ケース 1 件で契約を保護できるなら、
レスポンスフィールドごとにテストを作成する必要はありません。
