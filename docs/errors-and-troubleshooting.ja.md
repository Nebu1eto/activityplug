...
[English](errors-and-troubleshooting.md) |
[한국어](errors-and-troubleshooting.ko.md) | 日本語

ActivityPlug は型付きエラー契約として `ActivityPlugError` を使います。
同じコードがプロセス内サービス、公開 HTTP API、GraphQL API を通過します。
...
許可済み origin でもブロック対象アドレスに解決される場合があります。
デプロイが private ネットワークへの接続を意図している場合に限り
...
さい。選択したアダプターが要求したストリーミング capability を対応と
報告していることも確認してください。
...
トークン import は既定で無効です。アプリケーションに明示的な import
フローがある場合に限り `tokenImport.enabled: true` を設定してください。
...
更新中のネットワーク障害はログアウトの証拠ではありません。架空の匿名
セッションをキャッシュせず、プライベートキャッシュ状態を消去し、更新の
失敗を表示してください。
[ブラウザー統合](browser-integration.ja.md#認証の回復)を参照して
ください。
...
リクエストが想定されたものか確認する前に制限を引き上げないでください。
拒否するレイヤーとログに記録される理由を予測できるよう、proxy 制限を
ActivityPlug の制限に合わせてください。
