...
[English](deployment.md) | [한국어](deployment.ko.md) | 日本語
...
Caddy のローカル認証局を使います。インターネットへそのまま公開する構成
ではなく、本番構成に近いリファレンスとして扱ってください。
...
イメージの固定値と必須秘密情報を検証します。ほかの Compose 設定コマンド
は展開後の秘密情報を出力する可能性があるため `config --quiet` だけを
許可します。
...
失われてもよい場合に限り memory スタックを使います。PostgreSQL と Redis
...
秘密情報ごとにこのコマンドを個別に実行します。生成した値を追跡対象の
...
health endpoint の成功が示すのはプロセスと設定済みストアの準備完了
だけです。allowlist に含まれるすべての ActivityPub origin へ接続できる
ことや、すべてのアダプター操作が成功することまでは示しません。
...
古いデコーダが残る混在フリートは新しい cookie 形式を安定して処理
...
利用者が新しいブラウザーセッションを作成する必要があります。フリート
全体で一度にローテーションしてください。
