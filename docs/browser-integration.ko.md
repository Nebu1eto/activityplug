...
[English](browser-integration.md) | 한국어 | [日本語](browser-integration.ja.md)

ActivityPlug 브라우저 경계는 브라우저용 백엔드(BFF)입니다. ActivityPlug
인증 세션을 HttpOnly 쿠키 뒤에 보관하고, 동일 origin 웹 애플리케이션을
위해 더 작은 `/v1/browser/*` API를 노출합니다. 브라우저 코드는 기반
ActivityPlug 세션 ID를 수신하거나 제출하지 않습니다.
...
credential을 보호할 수 있는 네이티브 애플리케이션, 신뢰하는 서버, 기타
클라이언트는 공개 HTTP·GraphQL API를 대신 사용할 수 있습니다.
...
[세션 저장소](session-storage.ko.md)를 참고하십시오.
...
`unknown`을 사용합니다. 직접 연결된 Node 리스너는 보통 피어 주소를
제공합니다. 런타임이 안정적인 클라이언트별 식별자를 제공하지 못한다면
확인자를 제공하십시오. reverse proxy 뒤에서는 알려진 프록시 피어에서 온
전달 헤더만 허용하는 확인자를 제공하십시오.
...
콜백은 의도적인 CSRF 헤더 예외입니다. 외부 OAuth 리다이렉트는 사용자
정의 헤더를 제공할 수 없기 때문입니다. 대신 서버는 일회용 OAuth 상태
레코드를 점유하고, ActivityPlug 세션을 연결하기 전에 브라우저 세션 ID와
콜백 바인딩이 서명된 브라우저 쿠키와 일치하는지 검증합니다.
...
인증된 페이로드가 권위 있는 뷰어·capability 스냅숏입니다.
...
캐시하지 않은 채 갱신 오류를 표시합니다. 전송 실패는 서버 측 인증
상태를 확정하지 않습니다.
`examples/web-client/src/state/auth-recovery.ts` 구현은 겹치는 복구
시도를 합치고, 성공한 갱신을 인증·익명 클라이언트 상태 사이의 권위 있는
경계로 취급합니다.
...
브라우저 표면은 의도적으로 공개 HTTP·GraphQL API보다 작습니다. 선택한
어댑터가 지원하지 않는 작업은 capability 정보를 사용해 숨기거나
비활성화하십시오.
